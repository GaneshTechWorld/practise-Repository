...
public interface StudentRepo extends JpaRepository<Student, Serializable>, JpaSpecificationExecutor<Student> {
     default Specification<Student> findByCriteria(final StudentSearchDto studentRequestDto) {
        return (root, query, criteriaBuilder) -> {
            List<Predicate> predicateList = new ArrayList<>();

            if(StringUtils.isNotEmpty(studentRequestDto.getCourseName())) {
                ...
                predicateList.add(criteriaBuilder.like(caseInSensitiveForCourseName,studentRequestDto.getCourseName().toLowerCase()));
            }
            if(StringUtils.isNotEmpty(studentRequestDto.getStudentName())) {
                Expression<String> studentNameInTable = root.get("studentName");
                Expression<String> caseInsensitiveForStudentName = criteriaBuilder.lower(studentNameInTable);
                predicateList.add(criteriaBuilder.like(caseInsensitiveForStudentName,  studentRequestDto.getStudentName().toLowerCase() ));
            }
            query.distinct(true);
